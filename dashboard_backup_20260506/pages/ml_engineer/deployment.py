...
def _build_candidates(mapping: dict, local_map: dict) -> dict:
    ...
    for mlflow_name, info in mapping.items():
        if info["bc_status"] in ["SUBMITTED","UNKNOWN",""]:
            continue
...
def _run_deployment(info: dict, model_id: str, engineer: str):
    import httpx
    prog   = st.progress(0)
    status = st.empty()
    from utils.model_registry import _get_bc_status
    ...
    prog.progress(60)
    try:
        r = httpx.get(f"{API_URL}/model/hash",
...
def _render_deployed(name: str, info: dict, user: dict):
    import httpx, time
    bc_id     = info["bc_id"]
    mtype     = info["model_type"]
    auc       = info["auc_roc"]
